...
public class MultipleChoiceService {
    ...
    @Transactional
    public RestResponse createQuiz(HttpSession session, String title, List<Map<String, Object>> questions) {
        User uploader = AuthUtil.getUserFromSession(userRepository, session);

        MultipleChoice multipleChoice = MultipleChoice.createMultipleChoice(uploader, title);
        ...
        for (Map<String, Object> item : questions) {
            ...
            int answerIndex = (int) item.get("answerIndex");

            for (int i = 0; i < options.size(); i++) {
                MultipleChoiceOption option = MultipleChoiceOption.createMCOption(savedQuestion, options.get(i));
    ...
    @Transactional
    public RestResponse updateQuiz(HttpSession session, UUID uuid, String title) {
        User uploader = AuthUtil.getUserFromSession(userRepository, session);
        MultipleChoice multipleChoice = findQuizByUuid(uuid);
